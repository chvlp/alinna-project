                ...
                <div class="bg-gradient-primary">
                  <div class="row">
                    ...
                    <div style="padding-right: 3%" class="col-lg-6 col-5 my-auto text-end">
                        <a style="font-size: 27px;color:white" href="{{ route('admin.rent_room.index') }}">
                ...
                <tbody >
                    @foreach ($rent_room as $item)
                        <tr>
                                ...
                                <td class="align-middle text-center"> <span class="text-xs font-weight-bold">{{ $item->room_id }}</span></td>
      ...
      <div class="col-lg-4 col-md-4 mb-md-3 mb-3">
        <div class="card my-4">
          ...
          <div class="card-body" style="font-family: 'Noto sans Lao">
            <form action="{{ route('admin.rent_room.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
              <div >
                  ...
                  <select name="room_id" class="form-select form-select-sm" required>
                    <option class="text-danger" value=""> ເລືອກຫ້ອງ</option >
                      ...
                      {{-- <option value="{{$item->id}}"> {{$item->number}}</option> --}}
                        ...
                        @foreach ($rent_room as $rent_rooms)
                            @if ($item->id == $rent_rooms->room_id)
                              <option value="{{$item->id}}"> {{$item->number}}</option>
                            @endif
                        @endforeach
                      @endforeach
